...
`%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'
`Additional macro define by Roger Christman, September 1998'
`The following macros store a string into the stack'
`much like the initialize function in Chapter 5'
`It can be used to initiate different variables as well'
define(head, `ifelse(substr($1,0,1),\,"substr($1,0,2)","substr($1,0,1)")')
define(tail, `ifelse(substr($1,0,1),\,substr($1,2),substr($1,1))')
define(storestr, `	mov	head($1), %o0
	stub	%o0, [%fp + base + offset]
define(`offset', eval(offset+1)) ifelse(tail($1),,,`storestr(tail($1))') ')
define(initstr, `	define(`offset', 0) define(`base', $1)
storestr($2) ')
divert dnl
